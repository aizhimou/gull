.PHONY: build build-metal build-cuda build-nocgo build-whisper push version patch minor major
...
build:
	...
	go build -o $(BUILD_DIR)/vget-server .
...
build-metal:
	...
	C_INCLUDE_PATH="$(WHISPER_PATH)" \
	LIBRARY_PATH="$(WHISPER_PATH)" \
	go build -tags metal -o $(BUILD_DIR)/vget-server .
...
build-cuda:
	...
	go build -tags cuda -o $(BUILD_DIR)/vget-server .

# Build without CGO (uses embedded whisper.cpp binary)
build-nocgo:
	CGO_ENABLED=0 go build -o $(BUILD_DIR)/vget-server .
